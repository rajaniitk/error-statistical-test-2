...
class ColumnAnalysis:
    ...
    def _generate_numeric_categorical_insights(self, data1: pd.Series, data2: pd.Series, col1: str, col2: str) -> List[str]:
        ...
        summary_stats = num_data.groupby(cat_data).agg(['mean', 'median', 'std']).to_dict('index')

        # Check for differences in means/medians
        if len(summary_stats) > 1:
            means = [stats.fmean(summary_stats[cat]['mean']) for cat in summary_stats if 'mean' in summary_stats[cat] and summary_stats[cat]['mean'] is not None]
            if len(means) > 1:
                if max(means) - min(means) > 2 * np.mean([s['std'] for s in summary_stats.values() if 'std' in s and s['std'] is not None]):
                    insights.append(f"The mean of {num_col} varies significantly across categories of {cat_col}.")
